...
testthat::test_that("errors", {
  ...
  my_a_signal<-signal_averaging(mysignal,200,5)
  my_p<-power_determination(0.1,7,5,200,250,5,5)
  my_hats<-est_sig_hat(my_a_signal,100,0.2)
  my_stan_data<-(my_a_signal-my_hats[1])/my_hats[2]
  my_erp_range<-find_ERP_range(my_stan_data,2)
  my_estimations<-optimise_ERP(my_erp_range,my_a_signal[my_erp_range],250,100)
  ...
  testthat::expect_error(
    estimate_amplitude("wrong type"),
    "averaged_signal must be in vector form"
  )
  testthat::expect_error(
    est_sig_hat(my_a_signal,100,0.55),
    "buffer_pc is too large. Choose a value <0.45"
  )
  testthat::expect_error(
    optimise_ERP(my_erp_range,c(my_erp_range,22),250,100),
    "lengths of x and y must equal"
  )
  false_x <- my_erp_range[!my_erp_range %in% 100]
  testthat::expect_error(
    optimise_ERP(c(false_x,22),my_stan_data[my_erp_range],250,100),
    "x must contain peak centre"
  )
  testthat::expect_error(
   power_determination(0.1,-5,5,200,250,5,1),
  ...
  testthat::expect_output(power_determination(0.1,7,5,200,250,5,5))
})
